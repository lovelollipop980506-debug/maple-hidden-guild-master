...
export async function listMembers(opts: { limit: number; offset: number; sort?: string }) {
  ...
  const { data, count } = await supabaseAdmin()
    .from("users")
    .select(
      "discord_id, username, global_name, avatar, tier, total_points, member_status, character_name, level, job, last_login",
      { count: "exact" },
    )
...
export async function getMember(discordId: string) {
  ...
  const { data: user } = await db
    .from("users")
    .select(
      "discord_id, username, global_name, avatar, tier, roles, total_points, member_status, character_name, level, job, joined_at, last_login",
    )
